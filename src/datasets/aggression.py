...
class Aggression(MetaDataClass):

    labels = sorted([
        "aggression"
    ])

    columns = ["rev_id", "comment", "worker_id"] + labels
    ...
    def generate_prompt(cls, example: dict, personalized: bool, instruct: bool, generative: bool, n_shot: int) -> str:
        if generative:
            response_format_instruction = "Please compose your response as a single word \"true\" or \"false\"."
        else:
